...
async def tutor_agent(chat_session: ChatSession, file_path, user_input, time_tracking=None, deep_thinking=True):
    ...
    if time_tracking is None:
        ...
        # doc_length is the number of words for all documents combined
        doc_length = sum(len(doc.page_content.split()) for doc in _document) if _document else 0
    ...
    logger.info(f"File id: {file_id} with file length {doc_length} words\nTime tracking:\n{format_time_tracking(length=doc_length, time_tracking=time_tracking)}")
